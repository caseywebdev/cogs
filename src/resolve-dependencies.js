...
var getFiles = function (filePath, cb, files) {
  ...
  async.waterfall([
    ...
    function (file, cb) {
      files[filePath] = file;
      var filePaths = _.map(file.includes.concat(file.links), 'path');
...
var walk = function (filePath, type, files, visited) {
  ...
  var filePaths = _.map(file.includes, 'path').concat(
...
module.exports = function (filePath, cb) {
  async.waterfall([
    ...
    function (files, cb) {
      var includes = walk(filePath, 'includes', files);
      var links = walk(filePath, 'links', files);
      var globs = _.unique(_.flatten(_.map(includes.concat(links), 'globs')));
      cb(null, {includes: includes, links: links, globs: globs});
    }
  ], cb);
};

...
module.exports = herit(Engine, {
  ...
  process: function (asset, cb) {
    ...
    asset.source =
      ...
      "    define('" + name + "', [" + this.dependencies('module', "'", "'") +
        '], factory);\n' +
      '  }\n' +
  ...
  dependencies: function (attribute, prefix, suffix) {
    return _.map(this.options.dependencies, function (dependency) {
      return (prefix || '') + dependency[attribute] + (suffix || '');
    }).join(', ');
  }
});

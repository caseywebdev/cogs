...
var CONFIG_PATH = ARGV._[0] || 'cogs.json';
...
var bustCaches = function (filePath) {
  _.each(memoize.caches, function (cache) {
    _.each(cache, function (__, key) {
      if (minimatch(filePath, key)) delete cache[key];
    });
  });
};
...
var save = function (__, changedPath) {
  if (changedPath) {
    ...
    bustCaches(changedPath);
    if (changedPath === CONFIG_PATH) return loadConfig();
  }
  _.each(config.builds, function (targets, sourceGlob) {
    glob(sourceGlob, {nodir: true}, function (er, filePaths) {
      _.each(filePaths, function (filePath) {
        alert('info', filePath, 'Building...');
        var start = Date.now();
        saveBuild(filePath, sourceGlob, targets, config, function (er) {
...
var loadConfig = function () {
  try {
    ...
    config = require(RESOLVED_CONFIG_PATH);
  } catch (er) {
    return argvError("Unable to load '" + CONFIG_PATH + "'\n" + er);
  }
  config.watch ? initWatcher() : closeWatcher();
  save();
};

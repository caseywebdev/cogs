...
var Cli = module.exports = herit({
  ...
  start: function (config) {
    ...
    this.dependencies =
      _.reduce(this.env.manifest, function (dependencies, val, key) {
        dependencies[path.resolve(key)] =
          _.map(_.keys(val.dependencies), function (relative) {
    ...
    async.each(_.keys(targets), function (src, cb) {
      ...
      var statCache = glob(src, {stat: true}, function (er, files) {
        async.each(files, function (file, cb) {
          ...
          var isFirst = !self.firsts[file];
          self.firsts[file] = true;

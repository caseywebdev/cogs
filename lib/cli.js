...
var UglifyJs = require('./engines/uglify-js');
...
var Cli = module.exports = herit({
  ...
  startFromArgv: function () {
    ...
    if (_.isString(config.compressors)) {
      config.compressors =
        _.reduce(config.compressors.split(','), function (obj, kv) {
          var split = kv.split(':');
          obj[split[0]] = require('./engines/' + split[1]);
          return obj;
        }, {});
    }

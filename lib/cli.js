...
var Cli = module.exports = herit({
  ...
  save: function (file, src, dir, cb) {
    ...
    asset.build(function (er) {
      ...
      if (fingerprint) saves.push(async.apply(saveToDir, true));
  ...
  saveAll: function (ev, changed) {
    ...
    var exportFingerprints = this.config['export-fingerprints'];
    async.each(_.keys(targets), function (src, cb) {
      ...
      if (!exportFingerprints) return;
      var json = JSON.stringify(self.env.fingerprints, null, '  ');
      fs.writeFileSync(exportFingerprints, json);
    });
  },

  ...
  abs: function (logical, cb) {
    ...
    _.each(paths, function (dir, priority) {
      ...
      fs.readdir(dir, function (er, files) {
        if (er || priority > highestPriority) return done();
        var statsDone = _.after(files.length, done);
        var fileA = path.basename(check);
        _.each(files, function (fileB) {
          fs.stat(path.join(dir, fileB), function (er, stat) {
            if (er) return cb(er);
            var base = self.split(fileB).base;
            if (!stat.isDirectory() && (fileA === fileB || fileA === base)) {
              abs = path.join(dir, fileB);

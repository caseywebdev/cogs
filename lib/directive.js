...
module.exports = herit({
  ...
  isDirectory: function (asset, cb) {
    ...
    if (this.glob) {
      ...
      // A cache value of 1 means "Path exists, and is not a directory".
      // https://github.com/isaacs/node-glob#properties
      cb(this.glob.error, this.glob.statCache[asset.abs].isDirectory());
    } else {
  ...
  requireself: function (cb) {
    ...
    if (asset.env.amd.concat) {
      asset.amdDependencies(function (er, dependencies) {
        if (er) return cb(er);
        cb(null, dependencies.concat(asset));
      });
    } else {
      cb(null, asset);
    }
  ...
  requireglob: function (pattern, cb) {
    ...
    var self = this;
    this.glob = glob(target, {stat: true}, function (er, files) {
      if (er) return cb(er);

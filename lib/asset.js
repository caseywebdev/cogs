...
_.extend(Asset.prototype, {
  ...
  dependencies: function (cb, visited, required) {
    ...
    var ext = this.ext();
    async.eachSeries(this.directives, function (directive, cb) {
      async.eachSeries(directive.dependencies, function (dependency, cb) {
        var needsVisit = !_.contains(visited, dependency);
        if (needsVisit) return dependency.dependencies(cb, visited, required);
        var extsMatch = ext === dependency.ext();
        var addToRequired = !_.contains(required, dependency);
        if (extsMatch && addToRequired) required.push(dependency);
        cb();
      }, cb);
    }, function (er) {
      if (er) return cb(er);
  ...
  saveAs: function (target, cb) {
    ...
    this.build(function (er) {
      if (er) return cb(er);
      fs.writeFile(path.resolve(process.cwd(), target), self.toString(), cb);
    });
  },
  ...
  error: function (message) { return new Error(message + ' in ' + this.abs); }
});

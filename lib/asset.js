...
_.extend(Asset.prototype, {
  ...
  saveToDir: function (src, dir, fingerprint, cb) {
    ...
    mkdirp(dir, function (er) {
      ...
      if (fingerprint) {
        cb = _.after(2, cb);
        var fingerprinted = path.join(dir, self.filename(fingerprint));
        fs.writeFile(fingerprinted, buffer, cb);
        self.env.fingerprints[standard] = fingerprinted;
      }
      fs.writeFile(standard, buffer, cb);
    });
    return this;
  }
});

...
module.exports = herit({
  ...
  postProcess: function (cb) {
    if (this.ext() === 'js') {
      ...
      if (this.env.amd.concat) this.generateAst().addName();
    }
  ...
  generateAst: function () {
    try { this.ast = esprima.parse(this.source, {range: true}); }
    catch (er) {}
    return this;
  },
  ...
  addName: function () {
    var offset = 0;
    var name = "'" + this.name + "', ";
    _.each(this.calleeNodes(this.ast, 'define'), function (node) {
      var first = node.arguments[0];
      if (first && first.type !== 'Literal') {
        var i = first.range[0] + offset;
        offset += name;
        this.source = this.source.slice(0, i) + name + this.source.slice(i);
      }
    }, this);
    if (offset) this.generateAst();
    return this;
  },
  ...
  astNodes: function (node) {
    if (!_.isObject(node)) return [];
    return [node].concat(_.flatten(_.map(node, this.astNodes, this)));
  },

  calleeNodes: function (node, name) {
    return _.filter(this.astNodes(node), function (node) {
  ...
  requiredAmdIds: function () {
    ...
    _.each(this.calleeNodes(this.ast, 'define'), function (node) {
      _.each(node.arguments, function (argument) {
        ...
        // A literal means the argument.value is the name of the module.
        case 'Literal':
          return definedIds.push(argument.value);

        // An array expression means argument.elements is the dependency list.
        case 'ArrayExpression':
          requiredIds = requiredIds.concat(_.pluck(argument.elements, 'value'));
        }
      });
    });
  ...
  amdDependencies: function (cb) {
    var self = this;
    async.map(this.requiredAmdIds(), function (id, cb) {

...
var addNameRe = /(define\s*\(\s*)([^\s'"])/g;

var amdDefineRe = /define\s*\(\s*(?:['"](.*?)['"]\s*,\s*)?(?:\[([\s\S]*?)\])?/g;
...
module.exports = herit({
  ...
  addName: function () {
    var name = "'" + this.name + "', ";
    this.source = this.source.replace(addNameRe, '$1' + name + '$2');
    return this;
  },
  ...
  amdDefineMatches: function () {
    var match;
    var matches = [];
    while (match = amdDefineRe.exec(this.source)) matches.push(match);
    return matches;
  },

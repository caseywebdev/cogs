...
_.extend(Asset.prototype, {
  ...
  scanDependencies: function (cb, updated) {
    if (!updated) updated = [];
    var self = this;
    if (!_.contains(updated, this)) {
      return this.update(function (er) {
        if (er) return cb(er);
        updated.push(self);
        self.scanDependencies(cb, updated);
      });
    }
    async.map(this.directives, function (directive, cb) {
      async.map(directive.dependencies, function (dependency, cb) {
        var needsVisit = !_.contains(updated, dependency);
        if (needsVisit) return dependency.scanDependencies(cb, updated);
        cb(null, dependency);
      }, cb);
    }, function (er, dependencies) {
      if (er) return cb(er);
      cb(null, _.unique(_.flatten(dependencies)));
    });
    return this;
  },
  ...
  fingerprint: function (algorithm) {
    return crypto.createHash(algorithm).update(this.toBuffer()).digest('hex');
  },
  ...
  saveToDir: function (src, dir, fingerprint, cb) {
    ...
    dir = path.resolve(dir, path.dirname(path.relative(src, this.abs)));
    var self = this;
    mkdirp(dir, function (er) {
      ...
      if (fingerprint) {
        cb = _.after(2, cb);
        fs.writeFile(path.resolve(dir, self.filename(fingerprint)), buffer, cb);
      }
      fs.writeFile(path.resolve(dir, self.filename()), buffer, cb);
    });
    return this;
  }
});

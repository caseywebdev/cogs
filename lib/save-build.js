...
var cbQueue = require('./callback-queue');

module.exports = function (filePath, sourceGlob, targets, config, cb) {
  var cbQueueKey = [module.exports, filePath, sourceGlob, targets, config];
  if (cbQueue.push(cbQueueKey, cb)) return;
  async.waterfall([
    ...
    function (build, cb) {
      ...
      var notSaved = _.without(targetPaths, build.targetPaths);
  ...
  ], _.partial(cbQueue.call, cbQueueKey));
};
